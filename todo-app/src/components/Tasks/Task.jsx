...
const Task = ({id, text, onRemove, completed, list, onEdit, onComplete}) => {
  ...
  <div className="checkbox">
      <input onChange={onChangeCheckbox} className="checkbox__input" id={`task-${id}`} type='checkbox' checked={completed} />
        <label className="checkbox__label" htmlFor={`task-${id}`}>
           
        </label>
            <p className="tasks__text">{text}</p>
            <div className="tasks__items-row-actions">
        <div className="row-actions__container" onClick={() => onEdit(list.id, {id, text})}>
          <svg
            width="15"
            height="15"
            viewBox="0 0 15 15"
        ...
        <div className="row-actions__container" onClick={() => onRemove(list.id, id)}>
          <svg
            width="11"
            height="11"
            viewBox="0 0 11 11"
