...
function App() {
  ...
  const onEditTask = (listId, taskObj) => {
    ...
    setLists(newList);
      axios.patch('http://localhost:3001/tasks/' + taskObj.id, {text: newTaskText})
          .catch(() => {
          alert('Не вдалось оновити задачу');
      });
  
  }

  const onRemoveTask = (listId, taskId) => {
    if (window.confirm('Ви дійсно бажаєте видалити задачу?')) {
      const newList = lists.map(item => {
        if (item.id === listId) {
  ...
  useEffect(() => {
    ...
    if (lists) {
      const list = lists.find(lists => lists.id === Number(listId));
      setActiveItem(list);
    }
  ...
  return (
    <div className="todo">
      <div className="todo__sidebar">
        ...
        {lists ? (
          <List items={lists}
            ...
            onClickItem={lists => {
              navigate(`/lists/${lists.id}`, {replace: true})
            }}
      ...
      <Routes>
          <Route path='/' element={lists && lists.map(lists => (
            <Tasks
              key={lists.id}
              lists={lists}
              onAddTask={onAddTask}
